...
Before Encedo HEM perform any push notification iquery first needs to gather unique session key (a.ka. login first). This session key (technicly curve25519 public key) is valid 24h and is required to validate inquery from mantated HEM.
...
Every request from Encedo HEM that requires Mobile App action needs to be routed to them. By calling this endpoint all registered apps will get a notification about pending inquery. If <code>&lt;aid&gt;</code> is provided only that one mobile app will get notification, otherwise all registered.
...
Parameter | Description
--------- | -----------
eventid | Unique eventid generated by service (will be send to mobile app via FCM)
...
Parameter | Description
--------- | -----------
eventid | Unique eventid generated by service
...
<aside class="warning">
Parameters to send as POST data needs to be retrieve from Encedo HEM. Below is example usecase: Unlock of storage Flash drive</aside>
...
<aside class="warning">
If parameter <code>&lt;eventid&gt;</code> is omitted (or empty) mobile app will received PING notification. This is usefull to simulate a.ka. logon proccess by mobile app after launch.</aside>
...
Parameter | Description
--------- | -----------
pingid | Random ping id that will be send via FCM as well (to verify if FCM is working properly)
...
Object <code>&lt;act&gt;</code> contain action to perform. Object <code>&lt;ses&gt;</code> is unqie session id.
...
Mobile app based on user choose replace object <code>&lt;act&gt;</code> by appropriate reply (e.g. "RUS" -> "USA" if user choosed "Yes, read-only). Encdode structre back to base64 and digitally sign (procedure desiscribed upper).
...
Based on <code>&lt;data&gt;</code> if reply data needs to be return do HEM, this endpoint can be used. This phase is optional.
...
Parameter | Description
--------- | -----------
data | Data send by mobile app to HEM
sig | Signature od the data


### Response
Parameter | Description
--------- | -----------
status | Status of the inquery 
...
Signature <code>&lt;sig&gt;</code> is generated by mobile app and verify by HEM. It is used to validate return <code>&lt;data&gt;</code>. Generated is by following formula:

`key = ECDH( Base64Decode(aid_prv), Base64Decode(eid))`

`res = Base64Encode( HMAC(key, data) )`

Required <code>&lt;eid&gt;</code> is store in mobile app available via mapping <code>&lt;pid&gt;</code>. Mobile App after verifieng <code>&lt;sig&gt;</code> can begin processing request. Object  <code>&lt;data&gt;</code> after base64 decoding return embedded JSON structire encoding request type.
...
Data return by mobile app can be gathered by calling this endpoint and rerouted to HEM. This phase is optional.
...
Parameter | Description
--------- | -----------
eventid | The ID retrive during inquery of the request
